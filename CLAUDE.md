...
- **Languages:** English (`content/`, `content-deepdom/`) is the ONLY content
  humans/AIs edit directly. `content-it/`, `content-es/` … are generated by
  `scripts/translate_sync.py` (auto-run by `.github/workflows/translate.yml` on
  pushes to `test`) — never hand-edit them except to fix a bad translation, and
  know the next English edit of that article regenerates it.
