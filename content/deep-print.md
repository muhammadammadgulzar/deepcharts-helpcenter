...
The studio below is the **entire Deep Print dialog, alive** — every settings group from the reference table is wired to live candles. Switch **Type** between all four displays and flip **Profile/Box** mode. Filter the tape with **Min/Max Filter** and change **Input Type** to Aggregate Volume or Num Trades to watch every number recompute. **Zoom the price scale** (mouse wheel over the chart, or − / +) to feel Automatic tick grouping merge and split rows — then set grouping to Manual and zoom again: the rows freeze. Explore all four background **Color Modes** and five **Color Calculations**, single prints, bar-extreme ratios, volume clusters and pullback bars. Press **▶ Replay tape** to stream the trades back in one by one: cells grow, imbalances ignite and die, the POC migrates, and alerts toast (and optionally beep) when a print crosses your threshold. Flip on **◇ 3D tiles** and hover a single candle. When the chart looks like yours, hit **⬇ Download my settings** — the file lists every dialog section with your values, ready to replicate in DeepCharts.
