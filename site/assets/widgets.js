...
(function () {
  ...
  /* ================= DEEP PRINT STUDIO ================= */
  function deepPrintStudio(mount) {
    var body = chrome(mount, "Deep Print Studio — every setting live, export when it's yours", "FULL EDITOR");
    var st = { type: "split", mode: "profile", candles: 3, fontSize: 11, fmt: "K", dyn: true,
      imbOn: true, imbRatio: 3, imbMinDiff: 8, imbZero: false, domOnly: false,
      bidColor: "#7c5cff", askColor: "#22e56a", volColor: "#38d6ff",
      pocOn: true, dpocOn: false, dpocMin: 40, vaOn: true, vaPct: 70, threeD: false };

    /* ---- deterministic orderflow data ---- */
    var seed = 1337;
    function rnd() { seed = (seed * 16807) % 2147483647; return seed / 2147483647; }
    function genCandles(n) {
      seed = 1337; var out = [], px = 4500;
      for (var c = 0; c < n; c++) {
        var nlv = 11 + Math.floor(rnd() * 4), up = rnd() > 0.45, lv = [];
        for (var i = 0; i < nlv; i++) {
          var mid = 1 - Math.abs(i - nlv / 2) / (nlv / 2);
          var base = 8 + Math.floor(rnd() * 40 + mid * 90);
          var skew = up ? (i > nlv / 2 ? 1.7 : 0.8) : (i < nlv / 2 ? 1.7 : 0.8);
          lv.push({ p: px + (nlv - i) * 0.25,
            bid: Math.max(0, Math.floor(base * (2 - skew) * (0.5 + rnd()))),
            ask: Math.max(0, Math.floor(base * skew * (0.5 + rnd()))) });
        }
        if (rnd() > 0.5) lv[up ? 1 : nlv - 2][up ? "ask" : "bid"] = 0;
        out.push({ up: up, lv: lv }); px += up ? 1.5 : -1.25;
      }
      return out;
    }
    function fmtN(v) { return st.fmt === "K" && v >= 1000 ? (v / 1000).toFixed(1) + "K" : v; }
    function analyse(cd) {
      var tot = 0, poc = 0, dmax = 0, dpoc = -1;
      cd.lv.forEach(function (l, i) { var t = l.bid + l.ask; tot += t;
        if (t > (cd.lv[poc].bid + cd.lv[poc].ask)) poc = i;
        var d = Math.abs(l.ask - l.bid); if (d > dmax) { dmax = d; dpoc = i; } });
      var inVA = {}, cum = cd.lv[poc].bid + cd.lv[poc].ask, a = poc - 1, b = poc + 1; inVA[poc] = 1;
      while (cum < tot * st.vaPct / 100 && (a >= 0 || b < cd.lv.length)) {
        var va = a >= 0 ? cd.lv[a].bid + cd.lv[a].ask : -1, vb = b < cd.lv.length ? cd.lv[b].bid + cd.lv[b].ask : -1;
        if (va >= vb) { inVA[a] = 1; cum += va; a--; } else { inVA[b] = 1; cum += vb; b++; }
      }
      /* diagonal imbalance tests: ask vs bid one tick below; bid vs ask one tick above */
      cd.lv.forEach(function (l, i) {
        l.imbA = l.imbB = false;
        var below = cd.lv[i + 1], above = cd.lv[i - 1];
        if (below && (st.imbZero || below.bid > 0)) {
          if (l.ask - below.bid >= st.imbMinDiff && l.ask >= Math.max(below.bid, 1) * st.imbRatio) l.imbA = true;
        }
        if (above && (st.imbZero || above.ask > 0)) {
          if (l.bid - above.ask >= st.imbMinDiff && l.bid >= Math.max(above.ask, 1) * st.imbRatio) l.imbB = true;
        }
      });
      return { tot: tot, poc: poc, dpoc: dmax >= st.dpocMin ? dpoc : -1, inVA: inVA,
        delta: cd.lv.reduce(function (s, l) { return s + l.ask - l.bid; }, 0) };
    }

    body.innerHTML =
      '<div class="dps-grid" style="display:grid;grid-template-columns:280px 1fr;min-height:520px">' +
      '<div class="dps-ctl" style="border-right:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.015);padding:14px 14px 20px;overflow-y:auto;max-height:660px"></div>' +
      '<div style="padding:16px 18px 20px;display:flex;flex-direction:column;gap:12px">' +
      '<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">' +
      '<button class="dps-replay" style="background:rgba(124,92,255,.16);border:1px solid #7c5cff;color:#a58bff;border-radius:10px;padding:8px 15px;font:inherit;font-size:13px;font-weight:650;cursor:pointer">▶ Replay candles</button>' +
      '<button class="dps-3d" style="background:rgba(56,214,255,.1);border:1px solid rgba(56,214,255,.4);color:#38d6ff;border-radius:10px;padding:8px 15px;font:inherit;font-size:13px;font-weight:650;cursor:pointer">◇ 3D view: off</button>' +
      '<button class="dps-dl" style="margin-left:auto;background:#22e56a;border:none;color:#04140a;border-radius:10px;padding:9px 17px;font:inherit;font-size:13.5px;font-weight:750;cursor:pointer;box-shadow:0 0 26px rgba(34,229,106,.35)">⬇ Download my settings</button></div>' +
      '<div class="dps-stage" style="perspective:1100px"><div class="dps-tilt" style="transition:transform .5s cubic-bezier(.22,.8,.22,1)"><svg class="dps-svg" width="100%" viewBox="0 0 860 520"></svg></div></div>' +
      '<div class="dps-read" style="font-size:12.5px;color:#9aa3b2;line-height:1.7"></div></div></div>';
    if (matchMedia("(max-width:900px)").matches) body.querySelector(".dps-grid").style.gridTemplateColumns = "1fr";

    /* ---- settings panel (mirrors the real dialog groups) ---- */
    var ctl = body.querySelector(".dps-ctl");
    function grp(title, open) { return '<details ' + (open ? "open" : "") + ' style="border:1px solid rgba(255,255,255,.08);border-radius:11px;margin-bottom:9px;background:#101016"><summary style="cursor:pointer;padding:10px 12px;font-size:11.5px;letter-spacing:1.3px;text-transform:uppercase;color:#9aa3b2;font-weight:800;user-select:none">' + title + '</summary><div style="padding:4px 12px 13px;display:flex;flex-direction:column;gap:11px">'; }
    function sel(k, label, opts) { return '<label style="font-size:12.5px;color:#9aa3b2;display:flex;flex-direction:column;gap:5px">' + label + '<select data-k="' + k + '" style="background:#14141c;border:1px solid rgba(255,255,255,.15);border-radius:8px;color:#f2f4f8;padding:8px 10px;font:inherit;font-size:13px">' + opts.map(function (o) { return '<option value="' + o[0] + '"' + (st[k] === o[0] ? " selected" : "") + ">" + o[1] + "</option>"; }).join("") + "</select></label>"; }
    function rng(k, label, min, max, step, suf) { return '<label style="font-size:12.5px;color:#9aa3b2;display:flex;flex-direction:column;gap:5px"><span style="display:flex;justify-content:space-between">' + label + '<output style="color:#22e56a;font-weight:800">' + st[k] + (suf || "") + '</output></span><input type="range" data-k="' + k + '" min="' + min + '" max="' + max + '" step="' + (step || 1) + '" value="' + st[k] + '" data-suf="' + (suf || "") + '"></label>'; }
    function tgl(k, label) { return '<label style="font-size:13px;color:#c8d2de;display:flex;justify-content:space-between;align-items:center;cursor:pointer">' + label + '<input type="checkbox" data-k="' + k + '"' + (st[k] ? " checked" : "") + ' style="width:18px;height:18px;accent-color:#22e56a"></label>'; }
    function col(k, label) { return '<label style="font-size:13px;color:#c8d2de;display:flex;justify-content:space-between;align-items:center">' + label + '<input type="color" data-k="' + k + '" value="' + st[k] + '" style="width:34px;height:24px;border:none;background:none;cursor:pointer"></label>'; }
    ctl.innerHTML =
      grp("General settings", true) + sel("type", "Type", [["volume", "Volume"], ["split", "Ask/Bid Split"], ["delta", "Delta"], ["deltatotal", "Delta + Total Volume"]]) + sel("mode", "Mode", [["profile", "Profile"], ["box", "Box"]]) + rng("candles", "Candles shown", 1, 5) + "</div></details>" +
      grp("Text settings") + rng("fontSize", "Font Size", 8, 16) + sel("fmt", "Text Format", [["normal", "Normal"], ["K", "K (thousands)"]]) + tgl("dyn", "Dynamic Text Size") + "</div></details>" +
      grp("Background coloration (imbalance)", true) + tgl("imbOn", "Highlight imbalances") + rng("imbRatio", "Diagonal ratio ≥", 1.5, 6, 0.5, "×") + rng("imbMinDiff", "Min. Vol. Diff. For Imbalance", 0, 60) + tgl("imbZero", "Include Zero on Imbalance") + tgl("domOnly", "Color Only Dominant Side") + "</div></details>" +
      grp("Colors") + col("askColor", "Fixed-Ask Color") + col("bidColor", "Fixed-Bid Color") + col("volColor", "Fixed-Volume Color") + "</div></details>" +
      grp("POC") + tgl("pocOn", "Volume POC — Enable") + tgl("dpocOn", "Delta POC — Enable") + rng("dpocMin", "Delta POC Minimum Delta", 0, 120) + "</div></details>" +
      grp("Value Area") + tgl("vaOn", "Enable") + rng("vaPct", "% Value Area", 40, 95, 5, "%") + "</div></details>";

    /* ---- renderer ---- */
    var svg = body.querySelector(".dps-svg"), read = body.querySelector(".dps-read"), replayFlag = false;
    function draw() {
      var data = genCandles(st.candles);
      var rowH = Math.max(18, st.fontSize + 11), W = 860;
      var maxLv = Math.max.apply(null, data.map(function (c) { return c.lv.length; }));
      var H = Math.max(420, maxLv * rowH + 90); svg.setAttribute("viewBox", "0 0 " + W + " " + H);
      var cw = Math.min(240, (W - 60) / st.candles), pad = (W - cw * st.candles) / 2, out = "", ttips = [];
      var gmax = 0; data.forEach(function (c) { c.lv.forEach(function (l) { gmax = Math.max(gmax, l.bid + l.ask, l.bid, l.ask); }); });
      data.forEach(function (cd, ci) {
        var A = analyse(cd), x0 = pad + ci * cw + 10, w = cw - 20, y0 = 40 + (maxLv - cd.lv.length) * rowH / 2;
        out += '<line x1="' + (x0 - 6) + '" y1="' + y0 + '" x2="' + (x0 - 6) + '" y2="' + (y0 + cd.lv.length * rowH) + '" stroke="' + (cd.up ? st.askColor : st.bidColor) + '" stroke-width="2.5" opacity=".55"/>';
        cd.lv.forEach(function (l, i) {
          var y = y0 + i * rowH, tot = l.bid + l.ask, dl = l.ask - l.bid;
          var inVA = st.vaOn && A.inVA[i], dim = st.vaOn && !inVA ? .45 : 1;
          var anim = replayFlag ? ' style="animation:dpsin .5s cubic-bezier(.22,.8,.22,1) ' + ((cd.lv.length - i) * 0.07 + ci * 0.55).toFixed(2) + 's backwards"' : "";
          out += "<g" + anim + ' opacity="' + dim + '">';
          if (inVA) out += '<rect x="' + x0 + '" y="' + y + '" width="' + w + '" height="' + rowH + '" fill="rgba(255,255,255,.045)"/>';
          var fs = st.dyn ? Math.min(st.fontSize + 5, st.fontSize + Math.round(4 * tot / gmax)) : st.fontSize;
          function cell(cx, cwd, val, side, imb) {
            var color = side === "ask" ? st.askColor : side === "bid" ? st.bidColor : st.volColor;
            var inten = Math.min(1, (side === "vol" ? tot : (typeof val === "number" ? val : tot)) / gmax);
            if (st.mode === "profile") out += '<rect x="' + cx + '" y="' + (y + 2) + '" width="' + Math.max(2, cwd * inten).toFixed(1) + '" height="' + (rowH - 4) + '" rx="3" fill="' + color + '" opacity="' + (0.14 + inten * 0.35).toFixed(2) + '"/>';
            else out += '<rect x="' + cx + '" y="' + (y + 2) + '" width="' + cwd + '" height="' + (rowH - 4) + '" rx="3" fill="' + color + '" opacity="' + (0.10 + inten * 0.4).toFixed(2) + '"/>';
            if (imb && st.imbOn) out += '<rect x="' + cx + '" y="' + (y + 2) + '" width="' + cwd + '" height="' + (rowH - 4) + '" rx="3" fill="none" stroke="' + color + '" stroke-width="1.8" style="filter:drop-shadow(0 0 5px ' + color + ')"><animate attributeName="opacity" values="1;.45;1" dur="1.6s" repeatCount="indefinite"/></rect>';
            var bold = imb && st.imbOn ? ';font-weight:900' : (st.domOnly && !imb ? ";opacity:.55" : "");
            out += '<text x="' + (cx + cwd / 2) + '" y="' + (y + rowH / 2 + fs / 3) + '" text-anchor="middle" fill="#f2f4f8" font-size="' + fs + '" style="font-variant-numeric:tabular-nums' + bold + '">' + (typeof val === "number" ? fmtN(val) : val) + "</text>";
          }
          if (st.type === "split") { cell(x0, w / 2 - 1, l.bid, "bid", l.imbB); cell(x0 + w / 2 + 1, w / 2 - 1, l.ask, "ask", l.imbA); out += '<line x1="' + (x0 + w / 2) + '" y1="' + y + '" x2="' + (x0 + w / 2) + '" y2="' + (y + rowH) + '" stroke="rgba(255,255,255,.12)"/>'; }
          else if (st.type === "volume") cell(x0, w, tot, "vol", false);
          else if (st.type === "delta") cell(x0, w, (dl > 0 ? "+" : "") + fmtN(dl), dl >= 0 ? "ask" : "bid", l.imbA || l.imbB);
          else { cell(x0, w * 0.55 - 1, (dl > 0 ? "+" : "") + fmtN(dl), dl >= 0 ? "ask" : "bid", l.imbA || l.imbB); cell(x0 + w * 0.55 + 1, w * 0.45 - 1, tot, "vol", false); }
          if (st.pocOn && i === A.poc) out += '<rect x="' + (x0 - 1.5) + '" y="' + (y + .5) + '" width="' + (w + 3) + '" height="' + (rowH - 1) + '" rx="4" fill="none" stroke="#ffd35c" stroke-width="1.6"/>';
          if (st.dpocOn && i === A.dpoc) out += '<rect x="' + (x0 - 1.5) + '" y="' + (y + .5) + '" width="' + (w + 3) + '" height="' + (rowH - 1) + '" rx="4" fill="none" stroke="' + (dl >= 0 ? st.askColor : st.bidColor) + '" stroke-width="1.4" stroke-dasharray="5 4"/>';
          out += "</g>";
          ttips.push({ x: x0, y: y, w: w, h: rowH, l: l, A: A, i: i, price: l.p.toFixed(2) });
        });
        var fy = y0 + cd.lv.length * rowH + 16;
        out += '<text x="' + (x0 + w / 2) + '" y="' + fy + '" text-anchor="middle" fill="#6b7484" font-size="10.5" letter-spacing="1">Σ ' + fmtN(A.tot) + '   Δ <tspan fill="' + (A.delta >= 0 ? st.askColor : st.bidColor) + '" font-weight="800">' + (A.delta > 0 ? "+" : "") + A.delta + "</tspan></text>";
      });
      out += '<text x="16" y="20" fill="#6b7484" font-size="10" letter-spacing="2">DEEP PRINT — ' + { split: "ASK/BID SPLIT", volume: "VOLUME", delta: "DELTA", deltatotal: "DELTA + TOTAL VOLUME" }[st.type] + " · " + st.mode.toUpperCase() + " MODE (SIM DATA)</text>";
      svg.innerHTML = '<style>@keyframes dpsin{from{opacity:0;transform:translateY(8px) scale(.96)}}</style>' + out;
      replayFlag = false;
      var NS = "http://www.w3.org/2000/svg";
      ttips.forEach(function (t) {
        ...
        r.addEventListener("mousemove", function (e) {
          var dl = t.l.ask - t.l.bid, msg = "<b>" + t.price + "</b> — bid <b style='color:" + st.bidColor + "'>" + t.l.bid + "</b> × ask <b style='color:" + st.askColor + "'>" + t.l.ask + "</b> · Δ " + (dl > 0 ? "+" : "") + dl;
          if (t.l.imbA) msg += "<br><b>Ask imbalance</b> — buyers here beat the bid one tick below by ≥" + st.imbRatio + "× (diagonal test)";
          if (t.l.imbB) msg += "<br><b>Bid imbalance</b> — sellers here beat the ask one tick above by ≥" + st.imbRatio + "×";
          if (t.i === t.A.poc) msg += "<br><b style='color:#ffd35c'>Volume POC</b> — most traded price of this candle";
          showTip(msg, e.clientX, e.clientY);
        });
        r.addEventListener("mouseleave", hideTip);
        svg.appendChild(r);
      });
      var nImb = ttips.filter(function (t) { return t.l.imbA || t.l.imbB; }).length;
      read.innerHTML = "Hover any cell — it explains its bid×ask, delta and imbalance status. <b style='color:#f2f4f8'>" + nImb + " imbalanced level" + (nImb === 1 ? "" : "s") + "</b> at ratio ≥" + st.imbRatio + "× and min diff " + st.imbMinDiff + ". Gold border = Volume POC" + (st.vaOn ? "; dimmed rows sit outside the " + st.vaPct + "% value area" : "") + ".";
    }

    ctl.addEventListener("input", function (e) {
      var k = e.target.dataset.k; if (!k) return;
      if (e.target.type === "checkbox") st[k] = e.target.checked;
      else if (e.target.type === "range") { st[k] = +e.target.value; var o = e.target.closest("label").querySelector("output"); if (o) o.textContent = e.target.value + (e.target.dataset.suf || ""); }
      else st[k] = e.target.value;
      draw();
    });
    body.querySelector(".dps-replay").addEventListener("click", function () { replayFlag = true; draw(); });
    var tilt = body.querySelector(".dps-tilt"), stage = body.querySelector(".dps-stage"), btn3 = body.querySelector(".dps-3d");
    btn3.addEventListener("click", function () { st.threeD = !st.threeD; btn3.textContent = "◇ 3D view: " + (st.threeD ? "on" : "off"); btn3.style.background = st.threeD ? "rgba(56,214,255,.25)" : "rgba(56,214,255,.1)"; if (!st.threeD) tilt.style.transform = "none"; });
    stage.addEventListener("mousemove", function (e) {
      if (!st.threeD) return;
      var r = stage.getBoundingClientRect();
      var rx = -((e.clientY - r.top) / r.height - 0.5) * 10, ry = ((e.clientX - r.left) / r.width - 0.5) * 14;
      tilt.style.transform = "rotateX(" + rx.toFixed(1) + "deg) rotateY(" + ry.toFixed(1) + "deg)";
    });
    stage.addEventListener("mouseleave", function () { tilt.style.transform = "none"; });
    body.querySelector(".dps-dl").addEventListener("click", function () {
      var L = { volume: "Volume", split: "Ask/Bid Split", delta: "Delta", deltatotal: "Delta + Total Volume" };
      var txt = ["DEEP PRINT — MY SETTINGS (from the DeepCharts Help Center studio)",
        "Apply in: chart → Indicators → Deep Print → settings dialog", "",
        "[General settings]", "  Type: " + L[st.type], "  Mode: " + (st.mode === "profile" ? "Profile" : "Box"), "",
        "[Text settings]", "  Font Size: " + st.fontSize, "  Text Format: " + (st.fmt === "K" ? "K" : "Normal"), "  Dynamic Text Size: " + (st.dyn ? "On" : "Off"), "",
        "[Background coloration]", "  Color Calculation: Imbalance " + (st.imbOn ? "(highlighting on)" : "(off)"),
        "  Diagonal ratio used in studio: " + st.imbRatio + "x", "  Min. Vol. Diff. For Imbalance: " + st.imbMinDiff,
        "  Include Zero on Imbalance: " + (st.imbZero ? "On" : "Off"), "  Color Only Dominant Side: " + (st.domOnly ? "On" : "Off"), "",
        "[Colors]", "  Fixed-Ask Color: " + st.askColor, "  Fixed-Bid Color: " + st.bidColor, "  Fixed-Volume Color: " + st.volColor, "",
        "[POC]", "  Volume POC — Enable: " + (st.pocOn ? "On" : "Off"), "  Delta POC — Enable: " + (st.dpocOn ? "On" : "Off"), "  Delta POC Minimum Delta: " + st.dpocMin, "",
        "[Value Area]", "  Enable: " + (st.vaOn ? "On" : "Off"), "  % Value Area: " + st.vaPct, "",
        "JSON: " + JSON.stringify(st)].join("\n");
      var a = document.createElement("a");
      a.href = URL.createObjectURL(new Blob([txt], { type: "text/plain" }));
      a.download = "deep-print-my-settings.txt"; document.body.appendChild(a); a.click();
  ...
  var registry = {
    ...
    "deep-print-studio": deepPrintStudio,
  };
